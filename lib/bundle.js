...
/***/ function(module, exports, __webpack_require__) {
	...
	const Game = function (ctx){
	  ...
	  this.miss = new Audio('sounds/miss.wav');
	  this.miss.volume = this.soundEffectsLevel;
	  this.hit = new Audio('sounds/hit.wav');
	  this.hit.volume = this.soundEffectsLevel;
	  this.die = new Audio('sounds/die.wav');
	  this.die.volume = this.soundEffectsLevel;
	  this.clear = new Audio('sounds/clear.wav');
	  this.clear.volume = 0.5;
	}
	...
	Game.prototype.creatureMove = function (){
	...
	Game.prototype.action = function (action, x, y, x1, y1) {
	  switch (action) {
	    case 'ATTACK':
	      ...
	      if (((yourAttack + this.player.strBonus) > creature.ac) || yourAttack === 20){
	        this.hit.play();
	        ...
	        if (creature.stats.health <= 0) {
	          ...
	          this.player.kills += 1;
	          console.log([creature.stats.xp,this.currentLevel,this.player.level])
	      ...
	      if ((theirAttack + creature.stats.toHitBonus > this.player.ac) || theirAttack === 20) {
	...
	Game.prototype.removeCreature = function (creature) {
	  delete (this.map.monsterLocations[creature.id]);
	};
	...
	Game.prototype.introOutro = function (scenario, creature) {
	  ...
	  let text = '';
	  if (scenario === 'intro') {
	    text = "It all started with a harmless adventure into the cave, but soon misfortune struck. The earth started to shake and the exit out was buried in rubble. Now the only thing left to do is to try to find another way out...                        or die."
	  } else {
	    text = `You have died. You managed to make it to cave ${this.currentLevel}. On your way you killed ${this.player.kills} creatures but eventually met your fate as a level ${this.player.level} character fighting a ${creature}. Do you want to play again?`
	  }
	  let x = 0
	  let interval = setInterval(() => {
	    textBox.textContent += text[x]
	    x += 1
	    if (x >= text.length || introOutro.style.visibility === 'hidden') {
...
/***/ function(module, exports, __webpack_require__) {
	    ...
	    for (let j = 0; j < this.grid.length; j++) {
	      if (this.grid[i][j].type === '0') {
	        if (Math.random() < chanceForMonster) {
	          this.grid[i][j] = {type: 'M', seen: this.new ? false : true, seeing: false, variation: Math.floor(Math.random() * 3), creature: new Creature((Math.floor((Math.random() * this.level)/2) % 9), this.monsters,[i, j])}
	...
	Map.prototype.get=function(pos){
...
/***/ function(module, exports, __webpack_require__) {
	...
	const GameView = function (game, ctx, ctxUI, ctxDmg) {
	  ...
	  this.xUp = null;
	  this.yUp = null;
	  this.bgMusic = new Audio('sounds/adventure.mp3')
	  this.bgMusic.volume = 0.1;
	  this.bgMusic.addEventListener('ended', ()=>{
	    this.bgMusic.urrentTime = 0;
	    this.bgMusic.play();
	  }, false);
	};
	...
	GameView.prototype.fadeOutMusic = function (sound){
	...
	GameView.prototype.start = function () {
	  this.intro = new Audio('sounds/prelude.mp3');
	  this.intro.volume = 0.1;
	  this.intro.play();
	  this.lastTime = 0;
